...
type Group struct {
	...
	Members   json.RawMessage `gorm:"type:json" json:"members"`
	MemberCnt int             `gorm:"default:1" json:"memberCnt"`
	OwnerID   string          `gorm:"type:char(20);not null;index" json:"ownerId"`
	AddMode   int8            `gorm:"type:tinyint;default:0" json:"addMode"` // 0: direct join, 1: approval required
	Avatar    string          `gorm:"type:varchar(255);default:'https://api.dicebear.com/7.x/identicon/svg'" json:"avatar"`
	Status    int8            `gorm:"type:tinyint;default:0" json:"status"` // 0: active, 1: disabled, 2: dissolved
